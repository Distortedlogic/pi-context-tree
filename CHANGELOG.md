...
### Added
- **`/compress [instructions]`** — select one continuous safe range through two native Pi Session Tree screens, draft a summary with the current model, and require editor review before apply.
- **Native two-pass range selection** — native Enter selects the start and end in separate `TreeSelectorComponent` screens. Invalid or protected choices show a notice and reopen the relevant selector; atomic assistant-tool/result groups prevent unsafe boundaries.
...
- **Coverage** — core endpoint resolver tests, native selector command tests, extension apply/undo tests, a resumed real-session scenario, and a real-TUI `/compress` flow through Pi's Session Tree.

### Changed
- `/compress` now uses Pi 0.84.3's public native `TreeSelectorComponent`; the custom context panel no longer has a range view.
